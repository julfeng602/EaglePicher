...
|| '(select sum(l.buy_qty_due) from IFSAPP.customer_order_line l where l.order_no = o.order_no) ""' || ' Total Units ' || '"" , '
|| '(select sum(l.buy_qty_due * l.unit_price_incl_tax) from IFSAPP.customer_order_line l where l.order_no = o.order_no) ""' || ' Total Amount ' || '"" , '
|| 'l.line_no ""' || ' Line No ' || '"" , '
